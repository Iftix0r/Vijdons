...
@Composable
fun ChatGroupScreen(onBack: () -> Unit, viewModel: ChatGroupViewModel = hiltViewModel()) {
    ...
    Column(modifier = Modifier.fillMaxSize().background(VijdonColors.Background)) {
        Box(modifier = Modifier.padding(16.dp)) {
            ScreenHeader(title = "Barcha haydovchilar", subtitle = "Umumiy e'lon/xabar", onBack = onBack)
        }
        Box(modifier = Modifier.weight(1f).fillMaxWidth()) {
            when {
                ...
                state.messages.isEmpty() -> Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    ...
                    items(state.messages, key = { it.id }) { msg -> GroupBubble(msg) }
                }
            }
        }
        ...
        Row(
            ...
            OutlinedTextField(
                ...
                placeholder = { Text("Barchaga xabar...") },
                maxLines = 4,
                shape = ChipShape,
            ...
            IconButton(
                ...
                if (state.sending) {
                    ...
                    Icon(Icons.AutoMirrored.Rounded.Send, contentDescription = "Yuborish", tint = if (canSend) VijdonColors.TextOnBlue else VijdonColors.TextSecondary)
                }
            }
        }
    }
...
@Composable
private fun GroupBubble(msg: GroupMessageDto) {
    val isMe = !msg.is_driver
    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = if (isMe) Arrangement.End else Arrangement.Start) {
        Column(
            ...
            if (!isMe) {
                Text(msg.sender_name, color = VijdonColors.Green, style = MaterialTheme.typography.labelSmall.copy(fontWeight = FontWeight.Bold))
                Spacer(Modifier.height(2.dp))
            }
            Text(msg.text, color = if (isMe) VijdonColors.TextOnBlue else VijdonColors.TextPrimary, style = MaterialTheme.typography.bodyMedium)
            Spacer(Modifier.height(2.dp))
            Text(
                if (msg.created_at.length >= 16) msg.created_at.substring(11, 16) else msg.created_at,
