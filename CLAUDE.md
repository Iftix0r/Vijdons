...
- Drivers **cannot self-cancel** from the driver app — `driver_views.py` explicitly blocks `action == 'cancel'` and tells them to call `TariffSettings.operator_phone` instead. Cancellation is always operator-mediated by design; don't "fix" this without confirming it's actually wanted.
