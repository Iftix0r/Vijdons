...
# Mahalliy AddressQueueEntry navbat tizimidan mustaqil: bu yerda "manzil"
# o'rniga "yo'nalish" (Region → Region) bor, va bitta mashina bir nechta
# yo'lovchi tomonidan (har biri o'z narxida, joy-joy) to'ldiriladi.

@panel_login_required
def intercity_list(request):
    from .models import IntercityRoute, IntercityTrip, Region

    routes = IntercityRoute.objects.select_related('from_region', 'to_region').all()
    active_trips = (
        IntercityTrip.objects.filter(status=IntercityTrip.STATUS_FILLING)
        .select_related('route', 'route__from_region', 'route__to_region', 'driver')
        .prefetch_related('bookings')
        .order_by('-created_at')
    )
    regions = Region.objects.all()
    return render(request, 'taxi/intercity.html', {
        'routes': routes, 'active_trips': active_trips, 'regions': regions,
    })


@panel_login_required
@require_POST
def intercity_route_create(request):
    from .models import IntercityRoute, Region

    from_region = get_object_or_404(Region, pk=request.POST.get('from_region'))
    to_region = get_object_or_404(Region, pk=request.POST.get('to_region'))
    if from_region == to_region:
        messages.error(request, "Boshlanish va tugash viloyati bir xil bo'lishi mumkin emas.")
        return redirect('taxi:intercity_list')
    try:
        seat_price = float(request.POST.get('seat_price', 0))
        seat_capacity = int(request.POST.get('seat_capacity', 4))
    except (TypeError, ValueError):
        messages.error(request, "Narx va o'rindiqlar soni to'g'ri kiritilishi kerak.")
        return redirect('taxi:intercity_list')
    if IntercityRoute.objects.filter(from_region=from_region, to_region=to_region).exists():
        messages.error(request, f"«{from_region.name} → {to_region.name}» yo'nalishi allaqachon mavjud.")
        return redirect('taxi:intercity_list')
    IntercityRoute.objects.create(
        from_region=from_region, to_region=to_region,
        seat_price=seat_price, seat_capacity=max(1, seat_capacity),
    )
    messages.success(request, f"«{from_region.name} → {to_region.name}» yo'nalishi qo'shildi.")
    return redirect('taxi:intercity_list')


@panel_login_required
@require_POST
def intercity_route_update(request, pk):
    from .models import IntercityRoute

    route = get_object_or_404(IntercityRoute, pk=pk)
    try:
        route.seat_price = float(request.POST.get('seat_price', route.seat_price))
        route.seat_capacity = max(1, int(request.POST.get('seat_capacity', route.seat_capacity)))
    except (TypeError, ValueError):
        messages.error(request, "Narx va o'rindiqlar soni to'g'ri kiritilishi kerak.")
        return redirect('taxi:intercity_list')
    route.is_active = request.POST.get('is_active') == 'on'
    route.save(update_fields=['seat_price', 'seat_capacity', 'is_active'])
    messages.success(request, f"«{route}» yangilandi.")
    return redirect('taxi:intercity_list')


@panel_login_required
@require_POST
def intercity_route_delete(request, pk):
    from .models import IntercityRoute

    route = get_object_or_404(IntercityRoute, pk=pk)
    name = str(route)
    route.delete()
    messages.success(request, f"«{name}» yo'nalishi o'chirildi.")
    return redirect('taxi:intercity_list')


@panel_login_required
@require_POST
def intercity_booking_create(request):
    """Operator tomonidan telefon orqali kelgan mijoz uchun qo'lda joy
    band qilish — hozircha mijoz ilovasida o'z-o'zini ro'yxatga olish yo'q,
    shu sabab bu ish faoliyat uchun zarur birinchi bosqich."""
    from .models import IntercityRoute, IntercityTrip, IntercitySeatBooking

    route = get_object_or_404(IntercityRoute, pk=request.POST.get('route'))
    name = request.POST.get('passenger_name', '').strip()
    phone = request.POST.get('phone_number', '').strip()
    try:
        seats = max(1, int(request.POST.get('seats', 1)))
    except (TypeError, ValueError):
        seats = 1
    if not name or not phone:
        messages.error(request, "Ism va telefon raqami kiritilishi shart.")
        return redirect('taxi:intercity_list')

    # Joy bo'sh, "to'ldirilmoqda" holatidagi ENG OLDIN boshlangan safarga
    # qo'shiladi (adolatli navbat) — topilmasa yoki joy yetmasa, yangi
    # (hali haydovchisiz) safar ochiladi, keyin biror haydovchi ilovadan
    # qo'shilib olishi mumkin.
    trip = (
        IntercityTrip.objects.filter(route=route, status=IntercityTrip.STATUS_FILLING)
        .order_by('created_at')
        .first()
    )
    if trip is None or trip.seats_left < seats:
        trip = IntercityTrip.objects.create(route=route)
    IntercitySeatBooking.objects.create(
        trip=trip, passenger_name=name, phone_number=phone, seats=seats,
        price=route.seat_price * seats, created_by=request.user,
    )
    messages.success(request, f"«{name}» uchun {seats} ta joy band qilindi.")
    return redirect('taxi:intercity_list')


@panel_login_required
@require_POST
def intercity_booking_cancel(request, pk):
    from .models import IntercitySeatBooking

    booking = get_object_or_404(IntercitySeatBooking, pk=pk)
    booking.status = IntercitySeatBooking.STATUS_CANCELLED
    booking.save(update_fields=['status'])
    messages.success(request, 'Bandlik bekor qilindi.')
    return redirect('taxi:intercity_list')


@panel_login_required
@require_POST
def intercity_trip_depart(request, pk):
    from .models import IntercityTrip
    from django.utils import timezone

    trip = get_object_or_404(IntercityTrip, pk=pk)
    trip.status = IntercityTrip.STATUS_DEPARTED
    trip.departed_at = timezone.now()
    trip.save(update_fields=['status', 'departed_at'])
    messages.success(request, "Safar jo'natildi deb belgilandi.")
    return redirect('taxi:intercity_list')


@panel_login_required
@require_POST
def intercity_trip_cancel(request, pk):
    from .models import IntercityTrip

    trip = get_object_or_404(IntercityTrip, pk=pk)
    trip.status = IntercityTrip.STATUS_CANCELLED
    trip.save(update_fields=['status'])
    messages.success(request, 'Safar bekor qilindi.')
    return redirect('taxi:intercity_list')
