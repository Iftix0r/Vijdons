...
class IntercityRoute(models.Model):
    """Shahrlararo (viloyatlararo) yo'nalish — masalan 'Samarqand →
    Toshkent'. Operator sozlaydi: qaysi viloyatdan qaysi viloyatga, bitta
    o'rindiq narxi va mashinadagi yo'lovchi o'rinlari soni. Haqiqiy
    "yo'lovchi tashish" (bir nechta mijoz bitta mashinada, joy bo'yicha
    narx) tizimining asosi — mahalliy SavedAddress/AddressQueueEntry
    navbat tizimidan mustaqil (u yerda "manzil", bu yerda "yo'nalish")."""
    from_region   = models.ForeignKey(Region, on_delete=models.CASCADE, related_name='intercity_routes_from', verbose_name='Qayerdan (viloyat)')
    to_region     = models.ForeignKey(Region, on_delete=models.CASCADE, related_name='intercity_routes_to', verbose_name='Qayerga (viloyat)')
    seat_price    = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Bitta o'rindiq narxi (so'm)")
...
class IntercityTrip(models.Model):
    """Bitta yo'nalish bo'yicha hozir "to'lib borayotgan" (yoki allaqachon
    jo'nab ketgan) mashina. Haydovchi yo'nalishga "qo'shilganda" yaratiladi
    — taksi bekati navbati kabi, faqat manzil o'rniga yo'nalish. Yo'lovchilar
    shu safarga (bo'sh joy qolguncha) o'rindiq band qiladi."""
    STATUS_FILLING   = 'filling'
    STATUS_DEPARTED  = 'departed'
    STATUS_CANCELLED = 'cancelled'
    STATUS_CHOICES = (
        (STATUS_FILLING, "Joy to'ldirilmoqda"),
        (STATUS_DEPARTED, "Jo'nab ketdi"),
        (STATUS_CANCELLED, 'Bekor qilindi'),
    )
    route       = models.ForeignKey(IntercityRoute, on_delete=models.CASCADE, related_name='trips', verbose_name="Yo'nalish")
    driver      = models.ForeignKey(Driver, on_delete=models.SET_NULL, null=True, blank=True, related_name='intercity_trips', verbose_name='Haydovchi')
    status      = models.CharField(max_length=12, choices=STATUS_CHOICES, default=STATUS_FILLING, verbose_name='Holati')
    created_at  = models.DateTimeField(auto_now_add=True, verbose_name='Navbatga qo\'shilgan vaqt')
    departed_at = models.DateTimeField(null=True, blank=True, verbose_name="Jo'nagan vaqti")

    @property
    def seats_booked(self):
        return self.bookings.filter(status=IntercitySeatBooking.STATUS_CONFIRMED).aggregate(
            total=models.Sum('seats'),
        )['total'] or 0

    @property
    def seats_left(self):
        return max(0, self.route.seat_capacity - self.seats_booked)

    def __str__(self):
        return f'{self.route} — {self.driver or "haydovchisiz"} ({self.get_status_display()})'
...
class IntercitySeatBooking(models.Model):
    """Bitta yo'lovchining (yoki bir nechta birga ketayotgan yo'lovchining)
    biror IntercityTrip'dagi o'rindiq bandligi."""
    ...
    trip           = models.ForeignKey(IntercityTrip, on_delete=models.CASCADE, related_name='bookings', verbose_name='Safar')
    ...
    price          = models.DecimalField(max_digits=10, decimal_places=2, verbose_name='Narx', help_text="seats * IntercityRoute.seat_price (band qilingan paytdagi qiymat)")
    status         = models.CharField(max_length=12, choices=STATUS_CHOICES, default=STATUS_CONFIRMED, verbose_name='Holati')
    created_by     = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, verbose_name="Ro'yxatga olgan operator")
    created_at     = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f'{self.passenger_name} — {self.seats} joy ({self.trip})'
