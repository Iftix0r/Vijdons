...
def driver_order_action(request, driver, pk, action):
    ...
    if action == 'reject':
        # auto_reject_timeout (utils.py) xuddi shu buyurtmani/haydovchini
        # bir vaqtda qulflab turishi mumkin (masalan, timeout aynan shu
        # daqiqada ishga tushib, allaqachon boshqa haydovchiga (D2) qayta
        # yuborgan bo'lsa) — qulfsiz o'qish/yozish shu holatda eski
        # ('dispatched_to == biz') ma'lumotga asoslanib, D2'ga endigina
        # tayinlangan buyurtmani orqaga tortib olib, ikkinchi marta
        # qayta-dispatch qilib yuborardi. Shu sabab bu yerda ham
        # select_for_update bilan qulflanadi va DB'dan QAYTA o'qiladi.
        ...
        with transaction.atomic():
            ...
            if locked.status == 'pending':
                ...
                if should_redispatch:
                    ...
                    # navbatining OXIRIGA tushirilardi (endi javobsizlik/
                    # timeout holatida ham xuddi shunday tushirilmaydi —
                    # utils.py auto_reject_timeout()ga qarang). Endi "Rad
                    # etish" (yoki javob bermaslik) navbat o'rniga umuman
                    # TA'SIR qilmaydi — haydovchi navbatdagi o'rnini FAQAT
                    # oflaynga chiqqanda yoki buyurtma qabul qilganda
                    # yo'qotadi.
        if should_redispatch:
    ...
    # Haydovchi qabul qilingan buyurtmani endi o'zi bekor qila olmaydi —
    # bekor qilish uchun operatorga qo'ng'iroq qilishi kerak (frontend:
    # taxi/templates/driver/base.html'dagi cancelOrderCallOperator()).
    # Shu sababli bu amal serverda ham rad etiladi.
    if action == 'cancel':
